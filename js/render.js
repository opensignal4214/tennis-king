import { W, H, CX, CY, FOC, NETX, PRESS_LEAD, TOSS_APEX, HL, SW, DW, SVC, CHARGE_FULL, COLORS } from './constants.js';
import { G } from './state.js';
import { clamp, lerp, netHeight } from './utils.js';
import { facingAngle, bodyAnchors } from './facing.js';
import { armDrawOrder } from './pose.js';
import { proj } from './camera.js';
import { servingPlayer } from './scoring.js';
import { predictLanding } from './physics.js';

const cv  = document.getElementById('cv');
const ctx = cv.getContext('2d');
let RES = 1;

export function resizeCanvas() {
  const rect = cv.getBoundingClientRect();
  if (!rect.width) return;
  RES = clamp((rect.width * (window.devicePixelRatio || 1)) / W, 1, 3);
  const bw = Math.round(W * RES), bh = Math.round(H * RES);
  if (cv.width !== bw || cv.height !== bh) { cv.width = bw; cv.height = bh; }
}
window.addEventListener('resize', resizeCanvas);
resizeCanvas();

function quadWorld(x1, z1, x2, z2, color) {
  const a = proj(x1,0,z1), b = proj(x2,0,z1), c = proj(x2,0,z2), d = proj(x1,0,z2);
  if (!a||!b||!c||!d) return;
  ctx.fillStyle = color; ctx.beginPath();
  ctx.moveTo(a.x,a.y); ctx.lineTo(b.x,b.y); ctx.lineTo(c.x,c.y); ctx.lineTo(d.x,d.y);
  ctx.closePath(); ctx.fill();
}

function lineRect(x1, z1, x2, z2) {
  quadWorld(Math.min(x1,x2)-0.05, Math.min(z1,z2)-0.05, Math.max(x1,x2)+0.05, Math.max(z1,z2)+0.05, 'rgba(240,246,250,0.92)');
}

function drawCourt() {
  // Sky with subtle horizon warmth
  const sky = ctx.createLinearGradient(0,0,0,H*0.36);
  sky.addColorStop(0,'#0d1a2e'); sky.addColorStop(0.65,'#1d3a55'); sky.addColorStop(1,'#263f58');
  ctx.fillStyle = sky; ctx.fillRect(0,0,W,H*0.36);

  const wb = proj(0,0,-16);
  const wallBase = wb ? wb.y : 170;

  // Crowd stands — 4 rows of alternating seat clusters
  const rowH = 13, rowY0 = wallBase - 72;
  const rowCols = [['#1a3560','#15294e'],['#1c3a68','#172d55'],['#1e3e72','#18305c'],['#17305a','#122648']];
  for (let r = 0; r < 4; r++) {
    const y = rowY0 + r * rowH;
    const [ca, cb] = rowCols[r];
    for (let x = 0; x < W; x += 18) {
      ctx.fillStyle = (Math.floor(x/18) + r) % 2 === 0 ? ca : cb;
      ctx.fillRect(x, y, 18, rowH);
    }
    // Crowd heads: tiny ovals suggesting people
    ctx.fillStyle = `rgba(200,185,165,0.18)`;
    for (let x = 5; x < W; x += 18) ctx.fillRect(x, y + 1, 7, 4);
  }
  // Remaining wall strip below crowd
  ctx.fillStyle = '#12223a'; ctx.fillRect(0, wallBase-20, W, 20);
  ctx.fillStyle = '#0e3a2e'; ctx.fillRect(0, wallBase-14, W, 14);
  ctx.fillStyle = '#1c4a35'; ctx.fillRect(0, wallBase, W, H-wallBase);

  quadWorld(-13,-15.9,13,17.4,'#256648');
  quadWorld(-7.2,-14.2,7.2,16.8,'#2e7a55');
  quadWorld(-DW,-HL,DW,HL,'#2f6fb3');

  // Court surface radial gradient overlay — lighter center, darker edges
  const cc = proj(0,0,0), cf = proj(0,0,HL);
  if (cc && cf) {
    const cg = ctx.createRadialGradient(cc.x, cc.y, 0, cc.x, cc.y, Math.hypot(cc.x-cf.x, cc.y-cf.y)*1.3);
    cg.addColorStop(0,'rgba(80,130,200,0.12)'); cg.addColorStop(1,'rgba(0,0,0,0.10)');
    const a2 = proj(-DW,0,-HL), b2 = proj(DW,0,-HL), c2 = proj(DW,0,HL), d2 = proj(-DW,0,HL);
    if (a2&&b2&&c2&&d2) {
      ctx.fillStyle = cg; ctx.beginPath();
      ctx.moveTo(a2.x,a2.y); ctx.lineTo(b2.x,b2.y); ctx.lineTo(c2.x,c2.y); ctx.lineTo(d2.x,d2.y);
      ctx.closePath(); ctx.fill();
    }
  }

  // Bounce marks — faint ellipses at recent ball landing spots
  for (const bm of G.bounceMarks) {
    bm.age += 0.016;
    const bp = proj(bm.x, 0.01, bm.z); if (!bp) continue;
    const ba = Math.max(0, 0.32 - bm.age * 0.5);
    ctx.fillStyle = `rgba(255,255,255,${ba})`;
    ctx.beginPath(); ctx.ellipse(bp.x, bp.y, bp.s*0.22, bp.s*0.08, 0, 0, 7); ctx.fill();
  }

  lineRect(-DW,HL,DW,HL); lineRect(-DW,-HL,DW,-HL);
  lineRect(-DW,-HL,-DW,HL); lineRect(DW,-HL,DW,HL);
  lineRect(-SW,-HL,-SW,HL); lineRect(SW,-HL,SW,HL);
  lineRect(-SW,SVC,SW,SVC); lineRect(-SW,-SVC,SW,-SVC);
  lineRect(0,-SVC,0,SVC);
  lineRect(0,HL-0.3,0,HL); lineRect(0,-HL,0,-HL+0.3);
}

function drawNet() {
  const steps = 26, top = [], bot = [];
  for (let i = 0; i <= steps; i++) {
    const x = -NETX + i * (2*NETX/steps);
    top.push(proj(x, netHeight(x), 0)); bot.push(proj(x, 0, 0));
  }
  if (top.some(p => !p) || bot.some(p => !p)) return;
  ctx.beginPath();
  ctx.moveTo(top[0].x, top[0].y);
  top.forEach(p => ctx.lineTo(p.x, p.y));
  for (let i = steps; i >= 0; i--) ctx.lineTo(bot[i].x, bot[i].y);
  ctx.closePath();
  ctx.fillStyle = 'rgba(14,22,32,0.52)'; ctx.fill();
  ctx.strokeStyle = 'rgba(225,233,240,0.16)'; ctx.lineWidth = 1;
  for (let i = 0; i <= steps; i += 2) {
    ctx.beginPath(); ctx.moveTo(top[i].x,top[i].y); ctx.lineTo(bot[i].x,bot[i].y); ctx.stroke();
  }
  ctx.beginPath(); ctx.moveTo(top[0].x, top[0].y);
  top.forEach(p => ctx.lineTo(p.x, p.y));
  ctx.strokeStyle = '#eef3f7'; ctx.lineWidth = Math.max(2, top[Math.floor(steps/2)].s*0.085); ctx.stroke();
  [-NETX, NETX].forEach(x => {
    const t = proj(x, netHeight(x)+0.05, 0), g = proj(x, 0, 0);
    if (!t||!g) return;
    const pw = Math.max(3.5, g.s*0.075);
    // Post body
    ctx.fillStyle = '#1a2d40';
    ctx.fillRect(g.x - pw/2, t.y, pw, g.y - t.y);
    // Left highlight edge (cylinder illusion)
    ctx.fillStyle = 'rgba(255,255,255,0.13)';
    ctx.fillRect(g.x - pw/2, t.y, pw * 0.28, g.y - t.y);
    // Post cap
    ctx.fillStyle = '#b0bcc8';
    ctx.fillRect(g.x - pw/2 - 1, t.y - 3, pw + 2, 4);
  });
}

function drawShadow(x, z, r, alpha) {
  const p = proj(x, 0, z); if (!p) return;
  ctx.fillStyle = `rgba(0,0,0,${alpha})`;
  ctx.beginPath(); ctx.ellipse(p.x,p.y,r*p.s,r*p.s*0.32,0,0,7); ctx.fill();
}

function drawBall() {
  const b = G.ball;
  if (!b.active && !b.held) return;

  // Elongated directional trail — ovals oriented along travel direction
  for (let i = 0; i < b.trail.length; i++) {
    const t = b.trail[i], p = proj(t.x,t.y,t.z); if (!p) continue;
    const ni = b.trail[Math.min(i+1, b.trail.length-1)];
    const np = proj(ni.x,ni.y,ni.z);
    const a = (i / b.trail.length) * 0.32;
    ctx.fillStyle = `rgba(220,255,120,${a})`;
    if (np) {
      const dx = np.x - p.x, dy = np.y - p.y;
      const ang = Math.atan2(dy, dx);
      const len = Math.max(Math.hypot(dx, dy) * 0.65, p.s * 0.04);
      ctx.beginPath(); ctx.ellipse(p.x, p.y, Math.max(len, p.s*0.04), Math.max(1, p.s*0.032), ang, 0, 7); ctx.fill();
    } else {
      ctx.beginPath(); ctx.arc(p.x,p.y,Math.max(1,p.s*0.05),0,7); ctx.fill();
    }
  }

  drawShadow(b.x, b.z, 0.16, clamp(0.45-b.y*0.05, 0.08, 0.45));

  if (b.active && !b.held && b.vz > 0.5) {
    const L = predictLanding();
    if (L) {
      const inBounds = Math.abs(L.x) <= DW && L.z > 0 && L.z < HL + 0.15;
      const lp = proj(L.x, 0, L.z);
      if (lp) {
        ctx.strokeStyle = inBounds ? 'rgba(61,240,168,0.55)' : 'rgba(224,90,90,0.55)';
        ctx.lineWidth = 1.5;
        ctx.setLineDash([3, 4]);
        ctx.beginPath(); ctx.ellipse(lp.x, lp.y, lp.s*0.28, lp.s*0.10, 0, 0, 7); ctx.stroke();
        ctx.setLineDash([]);
      }
    }
  }

  const p = proj(b.x,b.y,b.z); if (!p) return;
  const r = Math.max(2.4, p.s*0.085);

  // Squash on bounce: briefly flatten horizontally
  const sq = b.squashT > 0 ? clamp(b.squashT / 0.14, 0, 1) : 0;
  const rx = r * (1 + sq * 0.42), ry = r * (1 - sq * 0.28);
  const gr = ctx.createRadialGradient(p.x-r*0.3,p.y-r*0.3,r*0.2,p.x,p.y,r);
  gr.addColorStop(0,'#f4ff9e'); gr.addColorStop(1,'#b8d018');
  ctx.fillStyle = gr;
  ctx.beginPath(); ctx.ellipse(p.x, p.y, rx, ry, 0, 0, 7); ctx.fill();

  // Hit flash: expanding ring at contact point
  if (b.hitFlash) {
    const ft = b.hitFlash.t / 0.28;
    const fp2 = proj(b.hitFlash.x, b.hitFlash.y, b.hitFlash.z);
    if (fp2) {
      const alpha = (1 - ft) * 0.75;
      const fr = r * (1 + ft * 4.0);
      ctx.strokeStyle = `rgba(255,255,200,${alpha})`;
      ctx.lineWidth = Math.max(1, (1-ft) * 2.8);
      ctx.beginPath(); ctx.arc(fp2.x, fp2.y, fr, 0, 7); ctx.stroke();
    }
  }
}

function drawChargeIndicator() {
  const p = G.player, ch = p.charge;
  if (!ch) return;
  const fp = proj(p.x, 0.05, p.z); if (!fp) return;
  const frac = clamp(ch.t / CHARGE_FULL, 0, 1);
  const st = G.strike;
  const inReach = st ? Math.abs(st.x - p.x) <= 1.6 : true;
  const err = st ? st.t - PRESS_LEAD : 1;     // time until the ideal release moment
  const inWin = st && inReach && Math.abs(err) <= 0.06;
  const R = Math.max(10, fp.s * 0.55), ry = R * 0.34;

  // base track
  ctx.strokeStyle = 'rgba(255,255,255,0.14)'; ctx.lineWidth = Math.max(3, fp.s * 0.05);
  ctx.beginPath(); ctx.ellipse(fp.x, fp.y, R, ry, 0, 0, 7); ctx.stroke();

  // charge-quality ring: amber building → lime → green at full power
  const chCol = frac >= 0.999 ? 'rgba(61,240,168,0.96)'
              : frac > 0.6    ? 'rgba(205,227,90,0.95)'
              :                 'rgba(232,180,80,0.92)';
  ctx.strokeStyle = chCol; ctx.lineWidth = Math.max(3, fp.s * 0.08);
  ctx.beginPath(); ctx.ellipse(fp.x, fp.y, R, ry, 0, -Math.PI / 2, -Math.PI / 2 + frac * Math.PI * 2); ctx.stroke();

  // timing ring: contracts toward the player as the ball nears the contact point
  if (st) {
    const Rt = R + 5 + Math.max(0, err) * fp.s * 2.2, rty = Rt * 0.34;
    if (!inReach) {
      ctx.strokeStyle = 'rgba(224,90,90,0.9)'; ctx.lineWidth = 2.4; ctx.setLineDash([5, 4]);
      ctx.beginPath(); ctx.ellipse(fp.x, fp.y, Rt, rty, 0, 0, 7); ctx.stroke(); ctx.setLineDash([]);
    } else {
      const tcol = inWin ? 'rgba(61,240,168,0.98)' : err > 0 ? 'rgba(232,232,232,0.82)' : 'rgba(224,160,90,0.92)';
      ctx.strokeStyle = tcol; ctx.lineWidth = inWin ? Math.max(3, fp.s * 0.07) : 2.4;
      ctx.beginPath(); ctx.ellipse(fp.x, fp.y, Rt, rty, 0, 0, 7); ctx.stroke();
    }
  }

  // labels: charge % on top, action cue below
  ctx.textAlign = 'center';
  ctx.fillStyle = chCol;
  ctx.font = `700 ${Math.max(10, fp.s * 0.17)}px "Avenir Next",sans-serif`;
  ctx.fillText(`${Math.round(frac * 100)}%`, fp.x, fp.y - ry - 6);
  const cueY = fp.y + ry + Math.max(13, fp.s * 0.2);
  if (st && !inReach) {
    ctx.fillStyle = 'rgba(224,90,90,0.96)'; ctx.fillText('REACH!', fp.x, cueY);
  } else if (inWin) {
    ctx.fillStyle = 'rgba(61,240,168,0.98)'; ctx.fillText('RELEASE', fp.x, cueY);
  }

  // Release pulse: expanding ring that fires when charge is released
  const rp = G.player.releasePulse;
  if (rp) {
    const rt = rp.t / 0.35;
    const rR = R * (1 + rt * 2.2), rRy = rR * 0.34;
    ctx.strokeStyle = `rgba(255,255,180,${(1-rt)*0.85})`;
    ctx.lineWidth = Math.max(1.5, (1-rt) * fp.s * 0.08);
    ctx.beginPath(); ctx.ellipse(fp.x, fp.y, rR, rRy, 0, 0, 7); ctx.stroke();
  }
}

function drawServeRing() {
  if (G.state !== 'toss' || !G.toss || G.toss.by !== 0 || G.toss.hit) return;
  const b = G.ball, p = proj(b.x,b.y,b.z); if (!p) return;
  const r = Math.max(2.4, p.s*0.085);
  const dtA = TOSS_APEX - G.toss.t;
  const inWin = Math.abs(G.toss.t - TOSS_APEX) <= 0.045;
  const col = inWin ? 'rgba(61,240,168,0.95)' : dtA > 0 ? 'rgba(232,232,232,0.8)' : 'rgba(224,160,90,0.9)';
  const R = r + 2 + Math.max(0, dtA) * 3.6 * p.s;
  ctx.strokeStyle = col; ctx.lineWidth = 2.2;
  ctx.beginPath(); ctx.arc(p.x,p.y,R,0,7); ctx.stroke();
}

let _lastRenderT = 0;

function drawParticles() {
  const now = performance.now() / 1000;
  const dt = _lastRenderT > 0 ? Math.min(now - _lastRenderT, 0.05) : 0.016;
  _lastRenderT = now;
  const alive = [];
  for (const pt of G.particles) {
    pt.age += dt;
    if (pt.age >= pt.maxAge) continue;
    pt.x += pt.vx * dt; pt.z += pt.vz * dt;
    pt.vx *= 0.84; pt.vz *= 0.84;
    alive.push(pt);
    const pp = proj(pt.x, 0.04, pt.z); if (!pp) continue;
    const pf2 = 1 - pt.age / pt.maxAge;
    const pr = Math.max(1, pp.s * 0.038 * pf2);
    ctx.fillStyle = `rgba(175,200,220,${pf2 * 0.55})`;
    ctx.beginPath(); ctx.ellipse(pp.x, pp.y, pr*1.7, pr*0.6, 0, 0, 7); ctx.fill();
  }
  G.particles.length = 0;
  for (const pt of alive) G.particles.push(pt);
}

function drawFb() {
  if (!G.fb) return;
  const p = proj(G.player.x, 2.15, G.player.z); if (!p) return;
  const age = G.fb.age, dur = 0.85;
  const fade = 1 - age / dur;
  // Scale-up: starts large (1.6x) and settles to 1x in first 0.12s, then fades
  const scaleT = Math.max(0, 1 - age / 0.12);
  const scale = 1 + scaleT * 0.6;
  const baseSize = Math.max(11, p.s * 0.3);
  ctx.globalAlpha = Math.max(0, fade);
  ctx.font = `700 ${baseSize * scale}px "Avenir Next",sans-serif`;
  ctx.textAlign = 'center';
  ctx.fillStyle = G.fb.col;
  ctx.shadowColor = 'rgba(0,0,0,0.8)'; ctx.shadowBlur = 6;
  ctx.fillText(G.fb.txt, p.x, p.y - age*34);
  ctx.shadowBlur = 0; ctx.globalAlpha = 1;
}

// Returns the visual entity index (0-3) for the server, accounting for singles (0/1) vs doubles (0-3)
function getServingEntityIdx() {
  const sv = servingPlayer();
  if (G.matchType === 'doubles') return sv;
  return sv === 0 ? 0 : 2; // singles: 0=player(entityIdx 0), 1=NPC(entityIdx 2)
}
function getTossingEntityIdx() {
  if (!G.toss) return -1;
  const by = G.toss.by;
  if (G.matchType === 'doubles') return by;
  return by === 0 ? 0 : 2; // singles: 0=player, 1=NPC→entityIdx 2
}

// entityIdx: 0=player, 1=partner, 2=npc, 3=npc2
function drawChar(e, colorKey, entityIdx) {
  const pf = proj(e.x,0,e.z), ph = proj(e.x,1.82,e.z);
  if (!pf||!ph) return;
  const s = pf.s, yAt = h => pf.y + (ph.y - pf.y) * (h / 1.82);
  const c = COLORS[colorKey] || COLORS.teal;
  const skin = '#e9b98c', shirt = c.shirt, shorts = c.shorts;
  const skinHi = '#f5cda0', skinLo = '#c99468';
  const shirtLo = c.shirtLo || shirt;
  const shortsLo = c.shortsLo || shorts;
  drawShadow(e.x, e.z, 0.42, 0.35);
  ctx.lineCap = 'round';
  const isHuman = entityIdx < 2;
  const fwd = isHuman ? -1 : 1;
  const dom = isHuman ? 1 : -1;
  const b = G.ball;
  const spd = e.spd || 0, stride = e.stride || 0;
  const amp = Math.min(0.3, 0.03 + spd*0.055);
  const stepLen = Math.min(0.45, 0.08 + spd*0.075);
  let dirx = 0, dirz = 0;
  if (spd > 0.25) { dirx = (e.vx||0)/spd; dirz = (e.vz||0)/spd; }
  const sw1 = Math.sin(stride), cw1 = Math.cos(stride);
  let px1 = -dirz, pz1 = dirx;
  // Foot A wires to the left hip; keep its stance offset pointing screen-left so the legs don't
  // cross when moving toward the net (where the raw perpendicular would flip to screen-right).
  if (px1 > 0) { px1 = -px1; pz1 = -pz1; }
  // Body facing: groundstrokes rotate the whole body side-on (¾ turn) and step the near foot
  // forward. bodyAnchors (js/facing.js, unit-tested) places every joint in world space for
  // this facing, so the upper and lower body share one facing and stay attached.
  const face = facingAngle(e.anim, isHuman);
  const lean = clamp(((e.lvx !== undefined ? e.lvx : 0) || e.vx || 0) * 0.03, -0.10, 0.10) * s;
  const A = bodyAnchors(e, face, fwd);
  // Project a world joint, applying a fraction `ln` of the body lean. Lean ramps 0 at the
  // planted feet → full at the shoulders so the body leans as one piece without detaching.
  const PJ = (pt, ln, fx, fy) => { const p = proj(pt.x, pt.y, pt.z); if (!p) return { x: fx, y: fy }; p.x += ln; return p; };
  const Lsh = PJ(A.shoulderL, lean,     pf.x-s*0.20+lean,     yAt(1.45));
  const Rsh = PJ(A.shoulderR, lean,     pf.x+s*0.20+lean,     yAt(1.45));
  const Lhip= PJ(A.hipL,      lean*0.4, pf.x-s*0.13+lean*0.4, yAt(0.97));
  const Rhip= PJ(A.hipR,      lean*0.4, pf.x+s*0.13+lean*0.4, yAt(0.97));
  const Lhb = PJ(A.hipBL,     lean*0.2, pf.x-s*0.11+lean*0.2, yAt(0.78));
  const Rhb = PJ(A.hipBR,     lean*0.2, pf.x+s*0.11+lean*0.2, yAt(0.78));
  // Shoulders/waist use the natural body-anchor widths (no broadening): a widened shoulder shelf
  // looks fine when the arms cover it (serve pose) but reads as a hard cutoff in the rally-ready
  // pose where both arms come forward and leave the outer shoulders exposed.
  // Feet: running uses the stride gait; otherwise the (turned) resting stance from bodyAnchors.
  let fpA, fpB;
  if (spd > 0.25) {
    const faM = { x: e.x + dirx*sw1*stepLen + px1*0.13, z: e.z + dirz*sw1*stepLen + pz1*0.13, l: Math.max(0, cw1)*amp*0.9 };
    const fbM = { x: e.x - dirx*sw1*stepLen - px1*0.13, z: e.z - dirz*sw1*stepLen - pz1*0.13, l: Math.max(0,-cw1)*amp*0.9 };
    fpA = proj(faM.x, faM.l, faM.z); fpB = proj(fbM.x, fbM.l, fbM.z);
  } else {
    fpA = proj(A.footA.x, 0, A.footA.z); fpB = proj(A.footB.x, 0, A.footB.z);
  }
  // Legs: filled tapered quads from each (rotated, lean-matched) lower-hip down to its foot,
  // so the legs stay joined to the shorts and turn with the body.
  const legQuad = (tx, ty, bx, by) => {
    const dx = bx - tx, dy = by - ty, L = Math.hypot(dx, dy) || 1;
    const nx = -dy / L, ny = dx / L;                 // unit perpendicular to the leg
    const wT = Math.max(1.8, s*0.058), wB = Math.max(1.4, s*0.042);
    // Cylinder shading across the leg width; lighter edge toward screen-left.
    const ax = tx + nx*wT, ay = ty + ny*wT, cx = tx - nx*wT, cy = ty - ny*wT;
    const g = ctx.createLinearGradient(ax, ay, cx, cy);
    const aLeft = ax <= cx;
    g.addColorStop(0, aLeft ? '#33485b' : '#131e28');
    g.addColorStop(1, aLeft ? '#131e28' : '#33485b');
    ctx.fillStyle = g;
    ctx.beginPath();
    ctx.moveTo(ax, ay);
    ctx.lineTo(cx, cy);
    ctx.lineTo(bx - nx*wB, by - ny*wB);
    ctx.lineTo(bx + nx*wB, by + ny*wB);
    ctx.closePath(); ctx.fill();
  };
  // Pull each leg-top toward the body centre so the legs sit directly under the shorts (the raw
  // lower-hip anchors splay wider than the waist, which made the legs read as detached).
  const midHbX = (Lhb.x + Rhb.x) / 2, midHbY = (Lhb.y + Rhb.y) / 2, legTopK = 0.68;
  const LlegX = lerp(midHbX, Lhb.x, legTopK), LlegY = lerp(midHbY, Lhb.y, legTopK);
  const RlegX = lerp(midHbX, Rhb.x, legTopK), RlegY = lerp(midHbY, Rhb.y, legTopK);
  if (fpA) legQuad(LlegX, LlegY, fpA.x, fpA.y);
  if (fpB) legQuad(RlegX, RlegY, fpB.x, fpB.y);
  // Shoes
  ctx.fillStyle = '#ddd8cc';
  if (fpA) { ctx.beginPath(); ctx.ellipse(fpA.x, fpA.y, Math.max(2.5, s*0.105), Math.max(1.4, s*0.042), 0, 0, 7); ctx.fill(); }
  if (fpB) { ctx.beginPath(); ctx.ellipse(fpB.x, fpB.y, Math.max(2.5, s*0.105), Math.max(1.4, s*0.042), 0, 0, 7); ctx.fill(); }

  const drawBody = () => {
    // Shorts: straight athletic panel from waist (0.97) to a hem just below the lower hips. The
    // sides drop vertically from the waist corners (no outward flare → reads as shorts, not a
    // skirt). Legs are painted first and the hem overlaps their tops, so they emerge from under it.
    const hemDrop = Math.max(1, s*0.025);
    const LhemX = Lhip.x, LhemY = Lhb.y + hemDrop;
    const RhemX = Rhip.x, RhemY = Rhb.y + hemDrop;
    const sg = ctx.createLinearGradient(0, (Lhip.y+Rhip.y)/2, 0, (LhemY+RhemY)/2);
    sg.addColorStop(0, shorts); sg.addColorStop(1, shortsLo);
    ctx.fillStyle = sg;
    ctx.beginPath();
    ctx.moveTo(Lhip.x, Lhip.y); ctx.lineTo(Rhip.x, Rhip.y);
    ctx.lineTo(RhemX, RhemY);   ctx.lineTo(LhemX, LhemY);
    ctx.closePath(); ctx.fill();
    // Inseam notch: a short split up the centre of the hem so the legs read as two panels.
    ctx.strokeStyle = shortsLo; ctx.lineWidth = Math.max(0.75, s*0.014);
    ctx.beginPath();
    ctx.moveTo((LhemX+RhemX)/2, (LhemY+RhemY)/2);
    ctx.lineTo((Lhip.x+Rhip.x)/2, lerp((Lhip.y+Rhip.y)/2, (LhemY+RhemY)/2, 0.4));
    ctx.stroke();
    // Torso: filled shirt panel from shoulders (1.45) to waist (0.97), rotated by facing.
    const tg = ctx.createLinearGradient(0, (Lsh.y+Rsh.y)/2, 0, (Lhip.y+Rhip.y)/2);
    tg.addColorStop(0, shirt); tg.addColorStop(1, shirtLo);
    ctx.fillStyle = tg;
    // Round the two shoulder corners so the shirt slopes into the arm (armhole curve) instead of
    // ending in a hard flat top edge with a sharp corner where the arm juts out.
    const unit = (ax, ay, bx, by) => { const dx=bx-ax, dy=by-ay, L=Math.hypot(dx,dy)||1; return {x:dx/L, y:dy/L}; };
    const rsh = Math.max(2, s*0.065);
    const dTop = unit(Lsh.x, Lsh.y, Rsh.x, Rsh.y);
    const dLs = unit(Lsh.x, Lsh.y, Lhip.x, Lhip.y);
    const dRs = unit(Rsh.x, Rsh.y, Rhip.x, Rhip.y);
    ctx.beginPath();
    ctx.moveTo(Lsh.x + dTop.x*rsh, Lsh.y + dTop.y*rsh);                  // top edge past the left corner
    ctx.lineTo(Rsh.x - dTop.x*rsh, Rsh.y - dTop.y*rsh);                  // top edge before the right corner
    ctx.quadraticCurveTo(Rsh.x, Rsh.y, Rsh.x + dRs.x*rsh, Rsh.y + dRs.y*rsh); // round right shoulder
    ctx.lineTo(Rhip.x, Rhip.y);
    ctx.lineTo(Lhip.x, Lhip.y);
    ctx.lineTo(Lsh.x + dLs.x*rsh, Lsh.y + dLs.y*rsh);                    // up the left side
    ctx.quadraticCurveTo(Lsh.x, Lsh.y, Lsh.x + dTop.x*rsh, Lsh.y + dTop.y*rsh); // round left shoulder
    ctx.closePath(); ctx.fill();
    const shAng = Math.atan2(Rsh.y-Lsh.y, Rsh.x-Lsh.x);
    // Head rides the projected shoulder midpoint: the perspective projection couples screen-x with
    // height via depth, so a foot-anchored head drifts sideways with court offset; the shoulders
    // keep it centred on the torso.
    const headX = (Lsh.x + Rsh.x) / 2;
    // Neck: a short skin column linking the shoulders to the head (kills the chibi no-neck look).
    // Drawn shaded so the jaw and collar read in front of it.
    const neckW = Math.max(1.6, s*0.045);
    ctx.fillStyle = skinLo;
    ctx.beginPath();
    ctx.moveTo(headX-neckW, yAt(1.49)); ctx.lineTo(headX+neckW, yAt(1.49));
    ctx.lineTo(headX+neckW*0.82, yAt(1.60)); ctx.lineTo(headX-neckW*0.82, yAt(1.60));
    ctx.closePath(); ctx.fill();
    // Crew collar: a tight darker band hugging the base of the neck.
    const cw = Math.max(2.2, neckW*2.1);
    ctx.fillStyle = shirtLo;
    ctx.beginPath(); ctx.ellipse(headX, (Lsh.y+Rsh.y)/2, cw, cw*0.5, shAng, 0, Math.PI); ctx.fill();
    ctx.fillStyle = skinLo;
    ctx.beginPath(); ctx.ellipse(headX, (Lsh.y+Rsh.y)/2, cw*0.6, cw*0.3, shAng, 0, Math.PI); ctx.fill();
    // Head: slightly smaller than before for adult-athletic (not big-headed chibi) proportions.
    const headR = Math.max(2.3, s*0.115);
    ctx.fillStyle = skin;
    ctx.beginPath(); ctx.arc(headX, yAt(1.63), headR, 0, 7); ctx.fill();
    ctx.fillStyle = isHuman ? '#222a30' : '#3a2417';
    ctx.beginPath(); ctx.arc(headX, yAt(1.67), headR*0.97, Math.PI, Math.PI*2); ctx.fill();
  };

  const KF = (A, B, u) => [lerp(A[0],B[0],u), lerp(A[1],B[1],u), lerp(A[2],B[2],u)];
  const a = e.anim;
  const tossing  = (G.state === 'toss'  && G.toss && getTossingEntityIdx() === entityIdx);
  const preServe = (G.state === 'serve' && getServingEntityIdx() === entityIdx);
  // bhPose: the dominant arm crosses the chest (a backhand) — ground stroke, volley, or the
  // ready/anticipation lean. Drives facing-aware occlusion below; idle/forehand/serve leave it false.
  let H2, T, off = null, two = false, E = null, Eo = null, shoulderTurn = 0, bhPose = false;
  if (tossing) {
    const u = clamp(G.toss.t / 0.45, 0, 1);
    H2 = [dom*0.32, 1.42, -fwd*0.3]; T = [dom*0.42, 2.0, -fwd*0.5];
    off = [-dom*0.22, lerp(1.15, 1.95, Math.min(1, u*1.3)), fwd*0.12];
  } else if (preServe) {
    H2 = [dom*0.3, 1.05, fwd*0.2]; T = [dom*0.36, 1.55, fwd*0.32];
    off = [-dom*0.24, 1.26, fwd*0.08];
  } else if (a) {
    const sd = a.side, fh = !isHuman ? sd < 0 : sd > 0;
    two = !fh && a.type === 'ground';
    const c = a.contact;
    if (a.type === 'smash' || a.type === 'serve') {
      const t = a.t;
      const He = c ? [c[0]*0.5, Math.max(1.5, c[1]-0.6), c[2]*0.5] : [dom*0.1, 1.9, fwd*0.2];
      const Te = c ? c : [dom*0.04, 2.2, fwd*0.45];
      if (t < 0.09) { const u = t/0.09;
        H2 = KF([dom*0.3,1.4,-fwd*0.3],[dom*0.3,1.55,-fwd*0.35],u);
        T  = KF([dom*0.4,1.95,-fwd*0.5],[dom*0.42,2.05,-fwd*0.55],u); }
      else if (t < 0.18) { const u = (t-0.09)/0.09, uu = u*u;
        H2 = KF([dom*0.3,1.55,-fwd*0.35],He,uu);
        T  = KF([dom*0.42,2.05,-fwd*0.55],Te,uu); }
      else { const u = Math.min(1,(t-0.18)/0.45);
        H2 = KF(He,[-dom*0.12,1.35,fwd*0.55],u);
        T  = KF(Te,[-dom*0.25,1.0,fwd*0.75],u); }
      off = [-dom*0.18, a.t<0.18?1.75:1.2, fwd*0.1];
    } else if (a.type === 'volley') {
      const u = Math.min(1, a.t/0.06);
      // Use actual contact side so the arm never crosses to the wrong side
      const cSide = c ? (Math.sign(c[0]) || sd) : sd;
      // Backhand volley = contact on the off side of the dominant hand → dominant arm crosses.
      bhPose = cSide * dom < 0;
      const He = c ? [c[0]*0.6, Math.max(1.0,c[1]-0.15), c[2]*0.6] : [cSide*0.55,1.18,fwd*0.5];
      // Constrain tip to 75% of contact offset to prevent shaft elongation on stretch volleys
      const Te = c ? [c[0]*0.75, Math.max(0.95,c[1]-0.1), c[2]*0.75] : [cSide*0.78,1.32,fwd*0.72];
      H2 = KF([cSide*0.42,1.32,fwd*0.08],He,u);
      T  = KF([cSide*0.62,1.55,fwd*0.18],Te,u);
      off = [-cSide*0.35,1.22,fwd*0.1];
    } else {
      const t = a.t;
      const rch = two ? 0.66 : 1.0;
      if (fh) {
        // Forehand: unit turn on take-back, racket drops below hand (eastern grip),
        // explosive swing through contact, full wrap-around follow-through.
        const Kb  = [sd*0.38, 1.00, -fwd*0.30];
        const KbT = [sd*0.54, 0.86, -fwd*0.42];
        const KbE = [sd*0.30, 0.96, -fwd*0.16];
        const Kc  = c ? [c[0]*0.58, Math.max(0.85,c[1]-0.18), c[2]*0.55] : [sd*0.52, 1.05, fwd*0.16];
        const KcT = c ? c : [sd*0.80, 1.10, fwd*0.40];
        const KcE = [sd*0.50, 0.90, fwd*0.02];
        const Kf  = [-sd*0.40, 1.80, fwd*0.36];
        const KfT = [-sd*0.42, 2.08, fwd*0.46];
        const KfE = [-sd*0.08, 1.56, fwd*0.28];
        if (a.charging) {
          H2=Kb; T=KbT; E=KbE; shoulderTurn=-sd*0.30;
          off = [-sd*0.26, 1.26, fwd*0.28];
        } else if (t<0.04) {
          H2=Kb; T=KbT; E=KbE; shoulderTurn=-sd*0.30;
          off = [-sd*0.26, 1.26, fwd*0.28];
        } else if (t<0.10) {
          const u=(t-0.04)/0.06, uu=u*u;
          H2=KF(Kb,Kc,uu); T=KF(KbT,KcT,uu); E=KF(KbE,KcE,uu);
          shoulderTurn = lerp(-sd*0.30, 0, uu);
          off = KF([-sd*0.38,1.32,-fwd*0.14], [-sd*0.35,1.20,fwd*0.04], uu);
        } else {
          const u=Math.min(1,(t-0.10)/0.55), uo=1-(1-u)*(1-u);
          H2=KF(Kc,Kf,uo); T=KF(KcT,KfT,uo); E=KF(KcE,KfE,uo);
          shoulderTurn = lerp(0, sd*0.35, uo);
          // off arm opens outward for counter-rotation balance
          off = KF([-sd*0.35,1.20,fwd*0.04], [-sd*0.52,1.44,fwd*0.46], uo);
        }
      } else {
        bhPose = true;
        // Backhand: shoulder coil toward ball side, higher finish than before.
        const Kb  = [sd*0.40*rch, 0.96, -fwd*0.28];
        const KbT = [sd*0.62*rch, 1.02, -fwd*0.44];
        const KbE = [sd*0.34*rch, 0.88, -fwd*0.14];
        const Kc  = c ? [c[0]*0.60*rch, Math.max(0.78,c[1]-0.18), c[2]*0.60*rch] : [sd*0.52*rch, 1.00, fwd*0.40];
        const KcT = c ? c : [sd*1.0, 1.05, fwd*0.62];
        const KcE = [sd*0.44*rch, 0.94, fwd*0.30];
        const Kf  = [-sd*0.28*rch, 1.65, fwd*0.44];
        const KfT = [-sd*0.52, 1.84, fwd*0.30];
        const KfE = [-sd*0.10*rch, 1.46, fwd*0.32];
        // Off (left/top) hand elbow — tucked near the torso so the rear arm reads as a second
        // hand on the grip instead of routing through the dominant elbow.
        const KbEo = [sd*0.14*rch, 0.92, -fwd*0.06];
        const KcEo = [sd*0.30*rch, 0.98, fwd*0.30];
        const KfEo = [-sd*0.04*rch, 1.34, fwd*0.16];
        if (a.charging) {
          H2=Kb; T=KbT; E=KbE; Eo=KbEo; shoulderTurn=sd*0.24;
        } else if (t<0.04) {
          H2=Kb; T=KbT; E=KbE; Eo=KbEo; shoulderTurn=sd*0.24;
        } else if (t<0.10) {
          const u=(t-0.04)/0.06, uu=u*u;
          H2=KF(Kb,Kc,uu); T=KF(KbT,KcT,uu); E=KF(KbE,KcE,uu); Eo=KF(KbEo,KcEo,uu);
          shoulderTurn = lerp(sd*0.24, 0, uu);
        } else {
          const u=Math.min(1,(t-0.10)/0.55), uo=1-(1-u)*(1-u);
          H2=KF(Kc,Kf,uo); T=KF(KcT,KfT,uo); E=KF(KcE,KfE,uo); Eo=KF(KcEo,KfEo,uo);
          shoulderTurn = lerp(0, -sd*0.30, uo);
        }
        if (!two) off = [-sd*0.45, 1.25, fwd*0.25];
      }
    }
  } else {
    let antSide = 0;
    if (G.state === 'live' && !b.held) {
      if (isHuman && b.lastHitter===1 && G.strike && G.strike.t<0.7) antSide = e.antSide||0;
      if (!isHuman && b.lastHitter===0 && b.z<2 && Math.hypot(b.x-e.x,b.z-e.z)<7) antSide = e.antSide||0;
    }
    if (antSide) {
      const fh = !isHuman ? antSide < 0 : antSide > 0;
      two = !fh;
      bhPose = !fh; // anticipating a backhand → dominant arm crosses, occlude it like the swing
      H2 = [antSide*0.42,0.98,-fwd*0.32]; T = [antSide*0.7,1.08,-fwd*0.58];
      if (!two) off = [-antSide*0.4,1.2,fwd*0.2];
    } else {
      H2 = [dom*0.14,1.02,fwd*0.32]; T = [dom*0.12,1.4,fwd*0.52]; two = true;
    }
  }
  const hp = proj(e.x+H2[0],H2[1],e.z+H2[2]), tp = proj(e.x+T[0],T[1],e.z+T[2]);
  if (!hp||!tp) { drawBody(); return; }
  hp.x += lean;
  const ep = E ? proj(e.x+E[0], E[1], e.z+E[2]) : null;
  if (ep) ep.x += lean;
  const epOff = Eo ? proj(e.x+Eo[0], Eo[1], e.z+Eo[2]) : null;
  if (epOff) epOff.x += lean;

  const shL = Lsh, shR = Rsh;
  const domSh = dom > 0 ? shR : shL;
  const offSh = dom > 0 ? shL : shR;
  const wSh = Math.max(1.9, s*0.052), wEl = Math.max(1.6, s*0.042), wHa = Math.max(1.3, s*0.03);
  const seg = (ax, ay, bx, by, wa, wb) => {
    const dx = bx-ax, dy = by-ay, L = Math.hypot(dx,dy)||1, nx = -dy/L, ny = dx/L;
    // Round the limb across its width: lit edge toward screen-left, shaded edge opposite.
    const e1x = ax+nx*wa, e1y = ay+ny*wa, e2x = ax-nx*wa, e2y = ay-ny*wa;
    const g = ctx.createLinearGradient(e1x, e1y, e2x, e2y);
    const e1Left = e1x <= e2x;
    g.addColorStop(0, e1Left ? skinHi : skinLo);
    g.addColorStop(1, e1Left ? skinLo : skinHi);
    ctx.fillStyle = g;
    ctx.beginPath();
    ctx.moveTo(e1x, e1y);
    ctx.lineTo(bx+nx*wb, by+ny*wb);
    ctx.lineTo(bx-nx*wb, by-ny*wb);
    ctx.lineTo(e2x, e2y);
    ctx.closePath(); ctx.fill();
  };
  const joint = (x, y, r) => { ctx.beginPath(); ctx.arc(x,y,r,0,7); ctx.fill(); };
  const drawArm = (sx, sy, elb) => {
    ctx.fillStyle = skin;
    joint(sx, sy, wSh);
    if (elb) { seg(sx,sy,elb.x,elb.y,wSh,wEl); joint(elb.x,elb.y,wEl); seg(elb.x,elb.y,hp.x,hp.y,wEl,wHa); }
    else seg(sx,sy,hp.x,hp.y,wSh,wHa);
  };
  const drawTrail = () => {
    if (a && a.t < 0.5) { (e._tt||(e._tt=[])).push({x:tp.x,y:tp.y}); if(e._tt.length>7)e._tt.shift(); }
    else if (e._tt && e._tt.length) e._tt.length = 0;
    if (e._tt && e._tt.length > 2) {
      ctx.strokeStyle = 'rgba(230,240,255,0.2)'; ctx.lineWidth = Math.max(2, s*0.1);
      ctx.beginPath(); ctx.moveTo(e._tt[0].x,e._tt[0].y);
      e._tt.forEach(q => ctx.lineTo(q.x,q.y)); ctx.stroke();
    }
  };
  const drawRacket = () => {
    const ang = Math.atan2(tp.y-hp.y, tp.x-hp.x);
    ctx.strokeStyle = '#caa36a'; ctx.lineWidth = Math.max(1.5, tp.s*0.05);
    ctx.beginPath(); ctx.moveTo(hp.x,hp.y); ctx.lineTo(tp.x,tp.y); ctx.stroke();
    const hx = tp.x + Math.cos(ang)*tp.s*0.15, hy = tp.y + Math.sin(ang)*tp.s*0.15;
    const headRx = Math.max(3.5, tp.s*0.155), headRy = Math.max(2.2, tp.s*0.10);
    ctx.fillStyle = 'rgba(215,228,240,0.22)';
    ctx.strokeStyle = '#dfe8ef'; ctx.lineWidth = Math.max(1.5, tp.s*0.05);
    ctx.beginPath(); ctx.ellipse(hx,hy,headRx,headRy,ang,0,7); ctx.fill(); ctx.stroke();
    const perp = ang + Math.PI/2;
    const cxa = Math.cos(ang), cya = Math.sin(ang), cxp = Math.cos(perp), cyp = Math.sin(perp);
    ctx.strokeStyle = 'rgba(255,255,255,0.30)'; ctx.lineWidth = Math.max(0.5, tp.s*0.018);
    for (let i = -1; i <= 1; i++) {
      const ox = cxp*i*headRy*0.48, oy = cyp*i*headRy*0.48;
      ctx.beginPath();
      ctx.moveTo(hx+ox-cxa*headRx*0.82, hy+oy-cya*headRx*0.82);
      ctx.lineTo(hx+ox+cxa*headRx*0.82, hy+oy+cya*headRx*0.82);
      ctx.stroke();
    }
    for (let i = -1; i <= 1; i++) {
      const ox = cxa*i*headRx*0.38, oy = cya*i*headRx*0.38;
      ctx.beginPath();
      ctx.moveTo(hx+ox-cxp*headRy*0.82, hy+oy-cyp*headRy*0.82);
      ctx.lineTo(hx+ox+cxp*headRy*0.82, hy+oy+cyp*headRy*0.82);
      ctx.stroke();
    }
  };
  const drawOffArm = () => {
    if (!off) return;
    const op = proj(e.x+off[0], off[1], e.z+off[2]); if (!op) return;
    op.x += lean; ctx.fillStyle = skin;
    joint(offSh.x, offSh.y, wSh); seg(offSh.x, offSh.y, op.x, op.y, wSh, wHa); joint(op.x, op.y, wHa);
  };

  const isGround = !!(a && a.type === 'ground');
  const fhOne = isGround && !two;        // forehand ground stroke (single grip, z-depth layering)
  const bhTwoHand = bhPose && two;        // two-handed backhand: ground stroke or ready/anticipation
  const bhOneHand = bhPose && !two;       // one-handed backhand: volley
  // Resolve the back-to-front paint order in js/pose.js (pure + unit-tested), then map each token
  // to its draw call. Keeping the layering logic out of here is what makes it testable.
  const kind = bhTwoHand ? 'bhTwo' : bhOneHand ? 'bhOne' : fhOne ? 'fhOne' : 'generic';
  const order = armDrawOrder({
    kind, isHuman, dom,
    handLat: H2[0], handZ: H2[2], offZ: off ? off[2] : null, two,
  });
  drawTrail();
  for (const step of order) {
    switch (step) {
      case 'body':     drawBody(); break;
      case 'domArm':   drawArm(domSh.x, domSh.y, ep); break;
      case 'offArm':   drawArm(offSh.x, offSh.y, epOff); break;
      case 'offBal':   drawOffArm(); break;
      case 'bothArms': drawArm(shL.x, shL.y, ep); drawArm(shR.x, shR.y, ep); break;
      case 'grip':     ctx.fillStyle = skin; joint(hp.x, hp.y, wHa); break;
      case 'racket':   drawRacket(); break;
    }
  }
}

function drawAimMarker() {
  if ((G.state !== 'serve' && G.state !== 'toss') || servingPlayer() !== 0) return;
  const p = proj(G.srvAim.x, 0, G.srvAim.z); if (!p) return;
  const pulse = 1 + 0.18 * Math.sin(performance.now() / 160);
  ctx.strokeStyle = 'rgba(90,231,208,0.85)'; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.ellipse(p.x,p.y,p.s*0.35*pulse,p.s*0.13*pulse,0,0,7); ctx.stroke();
  ctx.fillStyle = 'rgba(90,231,208,0.35)';
  ctx.beginPath(); ctx.ellipse(p.x,p.y,p.s*0.1,p.s*0.045,0,0,7); ctx.fill();
}

export function render() {
  ctx.setTransform(RES,0,0,RES,0,0);
  ctx.clearRect(0,0,W,H);
  drawCourt();
  drawAimMarker();

  // Build entity list — doubles adds partner (idx 1) and npc2 (idx 3)
  const allEntities = [
    { e: G.npc, ck: G.npcColors[0], idx: 2 },
    ...(G.matchType === 'doubles' && G.npc2 ? [{ e: G.npc2, ck: G.npcColors[1], idx: 3 }] : []),
    ...(G.matchType === 'doubles' && G.partner ? [{ e: G.partner, ck: G.partnerColor, idx: 1 }] : []),
    { e: G.player, ck: G.playerColor, idx: 0 },
  ].sort((a, b) => a.e.z - b.e.z); // ascending z = furthest from camera first (painter's algo)

  const behindNet = allEntities.filter(({ e }) => e.z < 0);
  const inFront   = allEntities.filter(({ e }) => e.z >= 0);

  behindNet.forEach(({ e, ck, idx }) => drawChar(e, ck, idx));
  if (G.ball.z < 0) drawBall();
  drawNet();
  if (G.ball.z >= 0) { drawBall(); drawServeRing(); }
  drawParticles();
  inFront.forEach(({ e, ck, idx }) => drawChar(e, ck, idx));

  if (G.state === 'live') drawChargeIndicator();
  drawFb();
  const v = ctx.createRadialGradient(CX,H*0.55,H*0.45,CX,H*0.55,H*0.95);
  v.addColorStop(0,'rgba(0,0,0,0)'); v.addColorStop(1,'rgba(0,0,0,0.28)');
  ctx.fillStyle = v; ctx.fillRect(0,0,W,H);
}
